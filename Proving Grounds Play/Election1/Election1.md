...
本筆記摘要自詳細報告 [[Election1-Walkthrough]]。以下是該靶機的快速滲透路徑：
...
*   **SUID 提權**：執行 `linpeas` 發現 `/usr/local/Serv-U` 檔案具有 SUID 權限且存在已知漏洞（[[suid#💥 Serv-U FTP Server 本地提權 (serv-u-ftp-server)|Serv-U FTP Server < 15.1.7 - CVE-2019-12181]]）。
*   **Exploit**：下載 `47173.sh` 本地提權腳本並執行，成功產出具有 root 權限的 `/tmp/sh`，進而獲取 `root` 權限與 `proof.txt`（亦可直接使用 [[pwnkit|PwnKit]] 提權）。
